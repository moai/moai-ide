...
namespace Moai.IDE.Cache
{
    public class AutoComplete : IAutoComplete
    {
        ...
        /// </summary>
        public IImageList Images
        {
            get
            {
                IImageList ret = Central.Platform.UI.CreateImageList();
                ret.Add(IDE.Resources.Images.autocomplete_namespace);
                ret.Add(IDE.Resources.Images.autocomplete_class);
                return ret;
            }
        }
        ...
        public void Import()
        {
            ...
            try
            {
                using (XmlReader reader = XmlReader.Create(PathHelpers.Sanitize(Central.Manager.Settings["RootPath"] + "/Completion Cache/root.xml")))
                {
    ...
    public class Item
    {
        ...
        private void Setup(string bas)
        {
            ...
            try
            {
                using (XmlReader reader = XmlReader.Create(PathHelpers.Sanitize(Central.Manager.Settings["RootPath"] + "/Completion Cache/" + bas + this.p_Name + ".xml")))
                {
                    t = Tree.FromXml(reader);
                }
            }
