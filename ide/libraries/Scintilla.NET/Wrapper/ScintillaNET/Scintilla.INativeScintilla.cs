...
namespace ScintillaNet
{
	public partial class Scintilla	
	{
        ...
        /// <param name="msg">Scintilla Message Number</param>
        /// <param name="wParam">int wParam</param>
        /// <param name="lParam">byte[] lParam</param>
        /// <returns></returns>
        unsafe int INativeScintilla.SendMessageDirect(uint msg, int wParam, byte[] lParam)
        {
            //  Just as when retrieving we make to convert .NET's
            //  UTF-16 strings into a document encoded byte array.
            fixed (byte* bp = lParam)
                return (int)_ns.SendMessageDirect(msg, (IntPtr)wParam, (IntPtr)bp);
        }
        ...
        void INativeScintilla.MarkerDefineRGBAImage(int markerNumber, byte[] pixels)
        {
            _ns.SendMessageDirect(Constants.SCI_MARKERDEFINERGBAIMAGE, markerNumber, pixels);
        }

        void INativeScintilla.RGBAImageSetWidth(int width)
        {
            _ns.SendMessageDirect(Constants.SCI_RGBAIMAGESETWIDTH, width);
        }

        void INativeScintilla.RGBAImageSetHeight(int height)
        {
            _ns.SendMessageDirect(Constants.SCI_RGBAIMAGESETHEIGHT, height);
        }
